...
let topMoviesTitle = document.createElement('h1')
topMoviesTitle.innerText = "Top movies"
...
searchBtn.onclick = function searchMovie(event) {
    ...
    let searchedMovie = topMovies.filter(movie => {
...
function filterByLanguages(currentLanguage) {
    row.innerHTML = ""
    let moviesByLanguages = topMovies.filter(movie => {
